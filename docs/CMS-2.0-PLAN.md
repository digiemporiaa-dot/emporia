...
per-page analytics are all operations _on content that cannot currently be
created_. Building them first would be building the second floor.
...
The sequence below is ordered by _what unblocks what_, not by the prompt's
numbering.
...
Delivered: `lib/services/schedule.service.ts` publishes through the same
`setPageStatus` path a person uses, so a scheduled publish takes a version and
writes an audit row. Publishes run before unpublishes so a window that elapsed
in full ends down. `publishAt` is cleared on firing (once only); `unpublishAt`
is kept, so a page put back by hand comes down again. ARCHIVED and soft-deleted
pages are never touched, one failure does not stop the rest, and the run is safe
to repeat. The endpoint returns **503 with no `CRON_SECRET`** rather than
...
_My recommendation: (c) now, (a) as its own phase later._
